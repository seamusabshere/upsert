      ...
      def execute(sql, params = nil)
        has_result = if params
          ...
          params.each_with_index do |v, i|
            if v.is_a?(Integer) && v > 2_147_483_647
        ...
        column_name_and_getter = (1..count).each_with_object({}) { |i, memo|
        ...
        while raw_result.next
          ...
          column_name_and_getter.each do |i, cg|
            ...
            row[column_name] = if getter == "getNull"
              nil
            elsif getter.respond_to?(:call)
              getter.call(raw_result, i)
            else
              raw_result.send(getter, i)
            end
          end
          result << row
        end
